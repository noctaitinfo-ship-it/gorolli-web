...
### F5.3 · MOBILE AUTO-OPEN (2026-07-03)
Owner decision: the active map auto-opens on **mobile too**. `reveal()` now runs unconditionally on load (still NO geolocation ask — desktop CTA remains the opt-in "use my location"; on mobile the CTA stays a plain same-tab link to /find). Mobile panel is now big and full-bleed: `height:82vh; min-height:620px; width:100vw` (negative-margin full-width, radius 0); desktop keeps `clamp(620px,78vh,900px)`. The **"Open full search" button moved OUT of the map entirely** — it is a normal-flow pill BELOW the panel (`insertAdjacentElement('afterend')`; on the root's desktop grid it sits under the map column, right-aligned), so it can never cover the client's bottom navigation or map content. Page scroll unchanged; no auto-redirects; iframe-fail fallback link and 9s loading state unchanged. Note: the black Google-Maps info-window issue is not fought via CSS — the tall panel gives it room; if it still misbehaves inside the embed, that's a client-app (Flutter web map) behavior to tune there.
